/**
 * src/_spa/DormDetailPage.tsx — Vite SPA 라우팅용 페이지
 *
 * ⚠️  이 파일은 src/App.tsx (react-router-dom) 전용입니다.
 *    Next.js App Router 라우트는 src/app/dorms/[id]/page.tsx 를 사용합니다.
    ...
    return (
      <div className="min-h-screen page-top flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/dorms" className="text-primary hover:underline">
            기숙사 목록으로 돌아가기
          </Link>
        </div>
      </div>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="glass-card-strong rounded-2xl p-6">
                ...
                <div className="flex items-center gap-8">
                  <ResponsiveContainer width={200} height={200}>
                    <PieChart>
                      <Pie data={chartData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={(entry) => `${entry.value}%`}>
            ...
            <div className="glass-card-strong rounded-2xl p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {facilities.map((f) => (
                  <div key={f.label} className={cn("flex items-center gap-2 p-3 rounded-xl border", f.available ? "bg-background/50 border-border/50" : "bg-muted/30 border-border/30 opacity-50")}>
                    <span className="text-xl">{f.icon}</span>
                    <span className={cn("text-sm font-medium", f.available ? "text-foreground" : "text-muted-foreground")}>{f.label}</span>
          ...
          <div className="space-y-6">
            ...
            <div className="glass-card-strong rounded-2xl p-5">
              ...
              {buildingInfo.note && <p className="text-xs text-muted-foreground mt-1">{buildingInfo.note}</p>}
            </div>
